...
typedef enum logic [3:0] {
	S_IDLE, S_SHIFT_W, S_WAIT_SHIFT_W, S_CAPTURE_W, S_FETCH_ACT, S_WAIT_ACT, S_MAC_ACC, S_RED_DONE_CHK, S_OUT_LOAD, S_OUT_SHIFT, S_WAIT_RELU, S_WAIT_REQUANT, S_OUT_DONE_CHK, S_DONE }
	state_t;
	...
	always_comb begin 
		state_n = state;
		unique case (state)
		S_IDLE: state_n = start ? S_SHIFT_W : S_IDLE;
		S_SHIFT_W: state_n = S_WAIT_SHIFT_W;
		S_WAIT_SHIFT_W: state_n = wload_done0 ? S_CAPTURE_W : S_WAIT_SHIFT_W;
		S_CAPTURE_W: state_n = S_FETCH_ACT;
		S_FETCH_ACT: state_n = S_WAIT_ACT; 
		S_WAIT_ACT: state_n = act_rd_valid ? S_MAC_ACC : S_WAIT_ACT;
		S_MAC_ACC: state_n = S_RED_DONE_CHK;
		S_RED_DONE_CHK: state_n = (red_step == cfg_reduction_len-1) ? S_OUT_LOAD : S_SHIFT_W;
		S_OUT_LOAD: state_n = S_OUT_SHIFT;
		S_OUT_SHIFT: state_n = S_WAIT_RELU;
		S_WAIT_RELU: state_n = relu_valid ? S_WAIT_REQUANT : S_WAIT_RELU;
		S_WAIT_REQUANT: state_n = requant_valid0 ? S_OUT_DONE_CHK : S_WAIT_REQUANT;
		S_OUT_DONE_CHK: state_n = (out_pe_cnt == numpe-1) ? S_DONE : S_OUT_SHIFT;
...
assign wload_start = (start == S_SHIFT_W);
...
assign act_rd_addr = act_addr_w'(red_step);
